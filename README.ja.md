...
- 線の交差、枠線重なり、箱貫通、文字はみ出しを検知する SVG lint
...
リポジトリには、通常の箱枠重なり用 `fixtures/border-overlap/...` と、大きいセクション枠用 `fixtures/large-frame-border-overlap/...` の回帰 fixture が含まれています。CI で配線崩れを拾いたいときに使えます。
